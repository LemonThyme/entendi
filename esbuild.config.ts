...
// --- Pre-bundle ext-apps App class for iframe injection ---
const appBridgeResult = await esbuild.build({
  entryPoints: [join('src', 'mcp', 'views', 'app-bridge.ts')],
  bundle: true,
  format: 'iife',
  globalName: 'McpApps',
  write: false,
...
const appBridgeCode = appBridgeResult.outputFiles![0].text;
writeFileSync(
  join('src', 'mcp', 'views', '_app-bridge-bundle.ts'),
  `// Auto-generated by esbuild.config.ts — do not edit\nexport const APP_BRIDGE_BUNDLE = ${JSON.stringify(appBridgeCode)};\n`
);
