...
const HOOK_SCRIPT = join(process.cwd(), 'plugin/hooks/session-init');

const SUITE_DIR = join(process.cwd(), '.test-tmp', `session-init-${randomBytes(4).toString('hex')}`);
...
describe('session-init hook (bash)', () => {
  beforeAll(() => {
  ...
  it('writes ENTENDI_API_KEY to CLAUDE_ENV_FILE when API key exists', () => {
    ...
    expect(envContent).toContain('ENTENDI_API_URL=');
  });
